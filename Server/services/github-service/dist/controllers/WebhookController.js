...
export class WebhookController {
    ...
    async handleInstallationEvent(payload) {
        ...
        if (action === 'created' || action === 'new_permissions_accepted') {
            await InstallationModel.findOneAndUpdate({ installationId: instData.id }, {
            ...
            await InstallationModel.findOneAndUpdate({ installationId: instData.id }, { status: 'suspended' });
            logger.info(`Suspended installation id ${instData.id}.`);
        }
        else if (action === 'unsuspend') {
            await InstallationModel.findOneAndUpdate({ installationId: instData.id }, { status: 'active' });
            logger.info(`Un-suspended installation id ${instData.id}.`);
        }
    }
    async handleInstallationReposEvent(payload) {
        const action = payload.action;
