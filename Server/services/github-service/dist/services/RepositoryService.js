...
export const findRepoDetails = async (owner, name) => {
...
export const syncRepositoriesForInstallation = async (installationId) => {
    ...
    for (const repo of gitHubRepos) {
        ...
        await RepositoryModel.findOneAndUpdate({ repoId: repo.id }, {
            installationId,
            name: repo.name,
    ...
    await RepositoryModel.updateMany({ installationId, repoId: { $nin: activeRepoIds } }, { isActive: false });
    return await RepositoryModel.find({ installationId, isActive: true });
};
export const toggleTriageRulesForRepo = async (fullName, active) => {
    const repo = await RepositoryModel.findOneAndUpdate({ fullName }, { triageRulesActive: active }, { new: true });
    if (!repo) {
