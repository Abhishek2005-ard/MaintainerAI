...
export const handleInstallationEvent = async (payload) => {
    ...
    if (action === 'created' || action === 'new_permissions_accepted') {
        await InstallationModel.findOneAndUpdate({ installationId: inst.id }, { accountName: inst.account.login, accountId: inst.account.id, accountType: inst.account.type, avatarUrl: inst.account.avatar_url, permissions: inst.permissions, status: 'active' }, { upsert: true, new: true });
