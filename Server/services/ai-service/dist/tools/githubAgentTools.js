...
export const getRepoInfoTool = tool(async ({ owner, repo }) => {
    const res = await fetch(`${base}/repos`, { headers: headers() });
    if (!res.ok)
        return `Failed to fetch repos (${res.status})`;
...
export const getIssuesTool = tool(async ({ owner, repo, state = 'open' }) => {
...
export const addLabelsTool = tool(async ({ owner, repo, issueNumber, labels }) => {
...
export const postCommentTool = tool(async ({ owner, repo, issueNumber, body }) => {
...
// Close an issue
export const closeIssueTool = tool(async ({ owner, repo, issueNumber }) => {
    const res = await fetch(`${base}/repos/${owner}/${repo}/issues/${issueNumber}`, {
...
export const reopenIssueTool = tool(async ({ owner, repo, issueNumber }) => {
    const res = await fetch(`${base}/repos/${owner}/${repo}/issues/${issueNumber}`, {
