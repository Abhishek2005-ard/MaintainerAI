...
const receiveIssueNode = async (state) => {
...
const fetchRepoContextNode = async (state) => {
...
const fetchSimilarIssuesNode = async (state) => {
    const { owner, repoName } = state.issue;
...
const generateEmbeddingNode = async (state) => {
...
const compareSimilarityNode = async (state) => {
    ...
    logger.info(`[Workflow:compareSimilarity] Best match: issue #${closestIssueNumber ?? 'none'} ` +
...
const routeAfterSimilarity = (state) => {
...
const markDuplicateNode = async (state) => {
...
const reasonWithLLMNode = async (state) => {
    const { title, body } = state.issue;
...
const predictLabelsNode = async (state) => {
...
const updateGitHubNode = async (state) => {
    const { owner, repoName, number } = state.issue;
...
const saveResultsNode = async (state) => {
    const { number, owner, repoName } = state.issue;
    logger.info(`[Workflow:saveResults] Triage complete for ${owner}/${repoName}#${number} — ` +
...
const notifyReportServiceNode = async (state) => {
...
export const runWorkflow = async (issue) => {
