...
import type { IssuePayload, TriageResult } from '../types/index.js';

// Validate the issue payload, run the full workflow, and return a clean typed result
export const runTriage = async (issue: IssuePayload): Promise<TriageResult> => {
  const requiredFields: (keyof IssuePayload)[] = ['issueId', 'number', 'title', 'body', 'owner', 'repoName'];
