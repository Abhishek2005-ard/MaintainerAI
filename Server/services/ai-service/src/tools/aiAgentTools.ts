...
const getLLM = () => {
  if (env.GEMINI_API_KEY) return new ChatGoogleGenerativeAI({ apiKey: env.GEMINI_API_KEY, model: 'gemini-2.0-flash' });
  if (env.OPENAI_API_KEY) return new ChatOpenAI({ apiKey: env.OPENAI_API_KEY, modelName: 'gpt-4o-mini', temperature: 0 });
  return null;
};
